...
async def main():
    ...
    {red}  本脚本根据GzGod的脚本修改而来，原作者：https://github.com/GzGod/
    {white}  支持多账号
    {white}  打包为exe文件，无需配置python环境
    {green}  请在userid.txt中输入您的用户ID和数量，格式为：userid:num
          """
    )
    user_datas = open("userid.txt", "r", encoding="utf-8").read().splitlines()
    if len(user_datas) <= 0:
    ...
    if not os.path.exists(args.proxy):
        ...
        exit()
    proxies = open(args.proxy, "r", encoding="utf-8").read().splitlines()
    print(proxies)
    # if len(proxies) <= 0:
    #     proxies = [None]
    ...
    for user_data in user_datas:
        # 把user_dat格式userid:num转为userid
        userid = user_data.split(":")[0]
        num = int(user_data.split(":")[1])
        for i in range(a, a+num):
            tasks.append(Grass(userid, proxies[i]).start())
        a += num
